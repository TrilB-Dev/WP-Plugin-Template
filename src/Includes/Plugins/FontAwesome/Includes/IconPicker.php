...
class IconPicker {
		...
		if ( function_exists( 'add_action' ) ) {
			$this->loader->register_component(
				...
				array(
					array(
						'type'     => 'action',
						'hook'     => 'wp_ajax_pluginname_fontawesome_search_icons',
						'callback' => 'ajax_search_icons',
					),
					array(
						'type'     => 'action',
						'hook'     => 'wp_ajax_pluginname_fontawesome_get_icon_data',
						'callback' => 'ajax_get_icon_data',
					),
				)
			)->run();
		}
	...
	public function get_icon_data( $icon_name, $pack = 'classic', $style = 'solid' ) {
		...
		try {
			...
			if ( function_exists( 'FortAwesome\findIconDefinition' ) ) {
				$definition = \FortAwesome\findIconDefinition(
					array(
						'prefix'   => $prefix,
						'iconName' => $icon_name,
					)
	...
	private function query_fontawesome_api( $pack, $style ) {
		...
		try {
			...
			foreach ( $all_icons as $icon_data ) {
				...
				// Build search terms array (empty for now since the search field is not available).
				$search_terms = array();
	...
	private function icon_matches_pack_style( $icon_data, $pack, $style ) {
		...
		return in_array( $membership_style, $free_styles, true ) || in_array( $membership_style, $pro_styles, true );
	}
				...
				echo wp_kses_post(
					FormFieldHelper::input(
						...
						array(
							'type'  => 'hidden',
							'id'    => $args['input_id'],
							'class' => 'pluginname-fa-picker-input',
						)
					)
				);
				?>
			...
			<div class="pluginname-fa-picker-modal" style="display: none;">
				...
				<div class="pluginname-fa-picker-dialog">
					...
					<div class="pluginname-fa-picker-search">
						...
						echo wp_kses_post(
							FormFieldHelper::input(
								'pluginname_fa_search',
								'',
								array(
									'type'        => 'search',
									'class'       => 'pluginname-fa-picker-search-input',
									'placeholder' => __( 'Search icons...', 'pluginname' ),
								)
							)
						);
						?>
						...
						echo wp_kses_post(
							FormFieldHelper::select(
								'pluginname_fa_pack',
								array(
									'classic'       => __( 'Classic', 'pluginname' ),
									'duotone'       => __( 'Duotone', 'pluginname' ),
									'sharp'         => __( 'Sharp', 'pluginname' ),
									'sharp-duotone' => __( 'Sharp Duotone', 'pluginname' ),
									'brands'        => __( 'Brands', 'pluginname' ),
								),
								'classic',
								array( 'class' => 'pluginname-fa-picker-pack-filter' )
							)
						);
						?>
						<?php
						echo wp_kses_post(
							FormFieldHelper::select(
								'pluginname_fa_style',
								array(
									'solid'   => __( 'Solid', 'pluginname' ),
									'regular' => __( 'Regular', 'pluginname' ),
									'light'   => __( 'Light', 'pluginname' ),
									'thin'    => __( 'Thin', 'pluginname' ),
								),
								'solid',
								array( 'class' => 'pluginname-fa-picker-style-filter' )
							)
						);
						?>
					</div>
	...
	private function parse_icon_value( $icon_value ) {
		...
		// Check if it is a full class string with a FontAwesome prefix.
		if ( preg_match( '/^(fa[sdrl][a-z]*)\s+fa-([a-z0-9-]+)$/', $icon_value, $matches ) ) {
			$prefix = $matches[1];
			$name   = $matches[2];

			// Convert the prefix back to the pack and style.
	...
	private function parse_fontawesome_prefix( $prefix ) {
		...
		return $reverse_map[ $prefix ] ?? array(
			'pack'  => 'classic',
			'style' => 'solid',
		);
	}
