 ...
 *
 * Shared utilities used by PluginName services, pages, and REST routes.
 *
 * @package TrilBDev
 * @subpackage Includes\PluginName\Functions
 * @since 1.0.0
 */
...
final class Functions {
	/**
	 * Default status for PluginName posts.
	 */
	public const DEFAULT_STATUS = 'publish';
	/**
	 * Allowed statuses for PluginName posts.
	 */
	public const ALLOWED_STATUSES = array( 'publish', 'draft', 'private' );
	/**
	 * Sanitizes a PluginName payload array for safe use.
	 *
	...
	/**
	 * Checks if a given post is a PluginName post.
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a PluginName post, false otherwise.
	 */
	public static function is_post( $post ): bool {
		return self::is_page( $post );
	}
	/**
	 * Checks if a given post is a PluginName page.
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a PluginName page, false otherwise.
	 */
	public static function is( $post ): bool {
		return PostHelper::is( $post );
	}
	/**
	 * Checks if a given post is a PluginName content (either a PluginName post or a PluginName page).
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a PluginName content, false otherwise.
	 */
	public static function is_page( $post ): bool {
		return PostHelper::is_page( $post );
	}
	/**
	 * Checks if a given post is either a PluginName post or a PluginName page.
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a PluginName content, false otherwise.
	 */
